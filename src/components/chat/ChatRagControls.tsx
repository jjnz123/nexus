...
import { RagScopeTimeRangeDialog, type RagTimeRangeResult } from "@/components/chat/RagScopeTimeRangeDialog";
...
const SCOPES_WITH_TIME_RANGE: RagSearchScope[] = ["meetings", "tasks"];
...
export function ChatRagControls({
  ...
  const [timeRangeScope, setTimeRangeScope] = useState<"meetings" | "tasks" | null>(null);
  ...
  function toggle(scope: RagSearchScope) {
    if (scopes.includes(scope)) {
      ...
      if (scope === "meetings") {
        onFiltersChange({ ...filters, meetingDateFrom: null, meetingDateTo: null });
      }
      if (scope === "tasks") {
        onFiltersChange({ ...filters, taskDateFrom: null, taskDateTo: null });
      }
      return;
    }

    if (SCOPES_WITH_TIME_RANGE.includes(scope)) {
      setTimeRangeScope(scope as "meetings" | "tasks");
      return;
    }
  ...
  function applyTimeRange(scope: RagSearchScope, range: RagTimeRangeResult) {
    if (scope === "meetings") {
      onFiltersChange({
        ...filters,
        meetingDateFrom: range.dateFrom,
        meetingDateTo: range.dateTo,
      });
    } else if (scope === "tasks") {
      onFiltersChange({
        ...filters,
        taskDateFrom: range.dateFrom,
        taskDateTo: range.dateTo,
      });
    }
    if (!scopes.includes(scope)) {
      onScopesChange([...scopes, scope]);
  ...
  function clearFilters() {
    onFiltersChange({
      kanbanProjectId: lockedProjectId ?? null,
  ...
  function scopeHint(scope: RagSearchScope) {
    if (scope === "meetings" && scopes.includes("meetings")) {
      if (!filters.meetingDateFrom && !filters.meetingDateTo) return "All time";
      if (filters.meetingDateFrom && filters.meetingDateTo) {
        return `${filters.meetingDateFrom.slice(0, 10)} → ${filters.meetingDateTo.slice(0, 10)}`;
      }
      return "Filtered";
    }
    if (scope === "tasks" && scopes.includes("tasks")) {
      if (!filters.taskDateFrom && !filters.taskDateTo) return "All time";
      if (filters.taskDateFrom && filters.taskDateTo) {
        return `${filters.taskDateFrom.slice(0, 10)} → ${filters.taskDateTo.slice(0, 10)}`;
      }
      return "Filtered";
    }
    return null;
  ...
  return (
    <>
      ...
      <RagScopeTimeRangeDialog
        open={timeRangeScope !== null}
        scope={timeRangeScope}
        onOpenChange={(open) => {
          if (!open) setTimeRangeScope(null);
        }}
        onConfirm={(scope, range) => {
          applyTimeRange(scope, range);
          setTimeRangeScope(null);
        }}
      />
    </>
  );
